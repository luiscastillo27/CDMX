...
var app = angular.module('RutasDetallesCtrl', []);
    ...
    app.controller('RutasDetallesCtrl', ['$scope', '$state', '$stateParams', '$ionicPopup', 'RutasBA', 'RutasMA', 'RutasMC', 'RutasTM', 'SeleBA', 'SeleMA', 'SeleMC', 'SeleTM', 'CruceBA', 'CruceMC', 'CruceMA', 'CruceTM', function ($scope, $state,  $stateParams, $ionicPopup, RutasBA, RutasMA, RutasMC, RutasTM, SeleBA, SeleMA, SeleMC, SeleTM, CruceBA, CruceMC, CruceMA, CruceTM) {
    ...
    $scope.cruce = function(){
        var jim;
        for(var m = 0; m < coordenX.length - 1 ; m++){
            for(var j = 0; j < coordenY.length - 1 ; j++){
                if(coordenX[j] > coordenX[j+1]){
                    aux = cruce[j];
                    cruce[j] = cruce[ j + 1 ];
                    cruce[j+1] = aux;
                }
